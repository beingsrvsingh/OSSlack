...
namespace Priest.Infrastructure.Service
{
    public class BookingClient : IBookingClient
    {
        ...
        public async Task<List<BookingResponseDto>> GetBookingsByDateAsync(int entityId, DateTime date)
        {
            try
            {
                var response = await _httpClient.GetFromJsonAsync<Result<IEnumerable<BookingResponseDto>>>($"booking/{entityId}/slots?date={date}");
