...
namespace Temple.Infrastructure.Persistence.EntityConfigurations
{
    public class ScheduleExceptionConfiguration : IEntityTypeConfiguration<ScheduleException>
    {
        public void Configure(EntityTypeBuilder<ScheduleException> builder)
        {
            ...
            // Columns
            builder.Property(se => se.TempleId)
            ...
            // Relationships
            builder.HasOne(se => se.Temple)
                   .WithMany(a => a.ScheduleExceptions) // assuming you don't have a ScheduleExceptions collection on Temple
                   .HasForeignKey(se => se.TempleId)
                   .OnDelete(DeleteBehavior.Cascade);

            // Optional: Index on TempleId + Date for faster lookup
            builder.HasIndex(se => new { se.TempleId, se.Date }).IsUnique(false);
        }
    }
}
