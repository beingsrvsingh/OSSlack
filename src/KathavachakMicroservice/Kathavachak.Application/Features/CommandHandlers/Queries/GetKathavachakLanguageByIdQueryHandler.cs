...
{
    public class GetKathavachakLanguageByIdQueryHandler : IRequestHandler<GetKathavachakLanguageByIdQuery, Result>
    {
        private readonly IKathavachakSessionModeService _service;
        private readonly ILoggerService<GetKathavachakLanguageByIdQueryHandler> _logger;

        public GetKathavachakLanguageByIdQueryHandler(
            IKathavachakSessionModeService service,
            ILoggerService<GetKathavachakLanguageByIdQueryHandler> logger)
        {
            _service = service;
            _logger = logger;
        }

        public async Task<Result> Handle(GetKathavachakLanguageByIdQuery request, CancellationToken cancellationToken)
        {
