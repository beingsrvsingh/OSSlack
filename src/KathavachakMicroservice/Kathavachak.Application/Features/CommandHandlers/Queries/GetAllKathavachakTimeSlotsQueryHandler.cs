...
{
    public class GetAllKathavachakTimeSlotsQueryHandler : IRequestHandler<GetAllKathavachakTimeSlotsQuery, Result>
    {
        private readonly IKathavachakTimeSlotService _service;
        private readonly ILoggerService<GetAllKathavachakTimeSlotsQueryHandler> _logger;

        public GetAllKathavachakTimeSlotsQueryHandler(
            IKathavachakTimeSlotService service,
            ILoggerService<GetAllKathavachakTimeSlotsQueryHandler> logger)
        {
            _service = service;
            _logger = logger;
        }

        public async Task<Result> Handle(GetAllKathavachakTimeSlotsQuery request, CancellationToken cancellationToken)
        {
