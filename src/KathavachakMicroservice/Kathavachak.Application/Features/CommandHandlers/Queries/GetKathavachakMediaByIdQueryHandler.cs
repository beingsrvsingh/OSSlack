...
{
    public class GetKathavachakMediaByIdQueryHandler : IRequestHandler<GetKathavachakMediaByIdQuery, Result>
    {
        private readonly IKathavachakSessionModeService _service;
        private readonly ILoggerService<GetKathavachakMediaByIdQueryHandler> _logger;

        public GetKathavachakMediaByIdQueryHandler(
            IKathavachakSessionModeService service,
            ILoggerService<GetKathavachakMediaByIdQueryHandler> logger)
        {
            _service = service;
            _logger = logger;
        }

        public async Task<Result> Handle(GetKathavachakMediaByIdQuery request, CancellationToken cancellationToken)
        {
