...
{
    public class GetAllKathavachakTopicsQueryHandler : IRequestHandler<GetAllKathavachakTopicsQuery, Result>
    {
        private readonly IKathavachakTimeSlotService _service;
        private readonly ILoggerService<GetAllKathavachakTopicsQueryHandler> _logger;

        public GetAllKathavachakTopicsQueryHandler(
            IKathavachakTimeSlotService service,
            ILoggerService<GetAllKathavachakTopicsQueryHandler> logger)
        {
            _service = service;
            _logger = logger;
        }

        public async Task<Result> Handle(GetAllKathavachakTopicsQuery request, CancellationToken cancellationToken)
        {
